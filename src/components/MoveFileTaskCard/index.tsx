...
import { MoveFileOutput, Task } from '../../api/task'
import { fileSize } from 'humanize-plus'
import useStyles from './style'

interface MoveFileTaskCardPropsType {
  className?: string
  task:Task<MoveFileOutput>
  onStop:() => void
}

const MoveFileTaskCard = ({ className, task, onStop }: MoveFileTaskCardPropsType): React.ReactElement => {
  ...
  const getTaskName = () => {
    if (task.output.files.length === 1) {
      ...
      return `${task.output.files[0].source.name} and other ${task.output.files.length - 1} tasks`
    }
  }
  return (
...
export default MoveFileTaskCard
